...
# The modifications do two things:
#	1. Accept RemotePath as an argument for `top`
#	2. yield DirEntry or RemotePath instead of string paths
...
def walk(top, topdown=True, onerror=None, followlinks=False):
	...
	if isinstance(top, str): ################## new
		top = Path(top) ################## new

	stack = [top] ################## fspath(top) -> top
	...
	while stack:
		...
		# left to visit.
		try:
			scanner = RemotePathScanner(top) if isinstance(top, RemotePath) else os.scandir(top) ################## new
			with scanner as entries: ################## scandir(top) -> scanner
				for entry in entries:
					...
					if is_dir:
						dirs.append(entry) ################## entry.name -> entry
					else:
		...
		if topdown:
			...
			for dirname in reversed(dirs):
				new_path = top / dirname.name ################## join(top, dirname) -> top / dirname.name
				# bpo-23605: path.islink() is used instead of caching
				# entry.is_symlink() result during the loop on os.scandir() because
				# the caller can replace the directory entry during the "yield"
				# above.
				if followlinks or not new_path.is_symlink(): ################## islink(new_path) -> new_path.is_symlink()
