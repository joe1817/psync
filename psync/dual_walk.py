...
P = TypeVar("P", bound=_AbstractPath)
...
class _Relpath:
	...
	def __post_init__(self):
		...
		object.__setattr__(self, "_hash", hash(self.relpath))
	...
	def __lt__(self, other:"_Relpath"):
		# TODO this is used to sort Operations, may remove it here
...
class _Diff:
	...
	def get_file_rename_map(self) -> dict[_File, _File]:
		...
		# return rename_map
		sorted_map = {k:rename_map[k] for k in sorted(rename_map.keys())} # TODO this won't be needed if the pattern in get_dir_rename_map is copied here
		return sorted_map
